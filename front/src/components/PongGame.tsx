...
const PongGame: React.FC<PongGameProps> = ({ mode, scoreToWin, onExit }) => {
    ...
    useEffect(() => {
        ...
        // 1. Conectar Socket (Asegúrate de que la URL del back es correcta)
        ...
        socket.onmessage = (event) => {
            try {
                ...
                if (msg.type === 'UPDATE') {
                    ...
                    // Actualizar Refs con datos del servidor
                    ...
                    if (s.status === 'playing' && isGameEndedRef.current) {
    ...
    return (
        <div style={{ 
            ...
            boxSizing: 'content-box' // <--- Fix del tamaño de caja
        }}>
            <canvas 
                ref={canvasRef} 
                width={CANVAS_WIDTH} 
            ...
            {/* OVERLAYS DE UI */}
            {uiState === 'loading' && (
                <div style={overlayStyle}><h1>CONNECTING...</h1></div>
            )}
